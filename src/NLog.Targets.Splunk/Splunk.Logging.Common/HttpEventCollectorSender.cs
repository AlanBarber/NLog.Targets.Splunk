...
using NLog.Targets.Splunk.Splunk.Logging.Common;
using System;
using System.Net;
...
namespace Splunk.Logging
{
    ...
    public class HttpEventCollectorSender : IDisposable
    {
        ...
        private readonly HttpEventCollectorSenderSettings httpSettings;
        ...
        public HttpEventCollectorSender(
          HttpEventCollectorSenderSettings httpSettings,
          string channel,
      ...
      httpMessageHandler.UseProxy = useProxy;
            ...
            byte[] serializedEvents)
        {
            ...
            try
            {
                ...
                HttpEventCollectorHandler next = (t, s) =>
                {
                    ...
                    var collectorPathUri = new Uri(this.httpSettings.RenderedUri, HttpEventCollectorPath);
                    return httpClient.PostAsync(collectorPathUri, content);
                };
                ...
                response = await postEvents(this.httpSettings.RenderedToken, serializedEvents);
        ...
        private void InitHttpClient()
        {
            ...
            try
            {
                var httpMessageHandler = BuildHttpMessageHandler(this.httpSettings.IgnoreSslErrors, this.httpSettings.UseProxy, this.httpSettings.MaxConnectionsPerServer);
            ...
            httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(AuthorizationHeaderScheme, this.httpSettings.RenderedToken);

            if (this.applyHttpVersion10Hack)
            {
                httpClient.BaseAddress = this.httpSettings.RenderedUri;
